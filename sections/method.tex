\section{Spatially Constrained Soft Routing}
\label{sec:method}
...
\caption{Main \mn{} dataflow at 224 pixels. All six local blocks run before
routing; every soft routing step retains 785 slots. The hard gather is the
first physical reduction. Recovery reads the final local features before
soft transport. Blue marks full-grid processing and orange marks compressed
processing. Numbers include the class token.}
...
weights $g_i$, with the step's scheduled merge budget as its target. We use $\tau_1=1$ and
multiplicative soft-selection scale 30. The released threshold routine uses
a batch-wide range normalization (Section~\ref{sec:batch}); its requested
budget does not guarantee exact relaxed selection mass for every numerical
input. The
...
not differentiate every gather index. This multiplier is computed at
evaluation too and equals one for finite values, whereas non-finite values
do not cancel. During training an auxiliary head contribution
...
tokens, with no further merging. Latent attention retains proportional
attention: it adds the logarithm of the selected token mass to key logits
before the softmax. The class representation feeds the classification head.
...
\paragraph{Reserved method variants.}
The original method above remains the subject of the completed experiments.
The initialized and progressive-latent variants in
Sections~\ref{sec:pending-init} and~\ref{sec:pending-progressive} have
separate pending endpoints. They do not retrospectively change the original
dataflow or its interpretation.

\input{sections/pending/visualization}

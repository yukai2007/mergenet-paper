...
final architecture in the collaborator deck and the executed classifier. The
deck also depicts a generic local-decoder branch; in the executed ImageNet
classifier, this branch is instantiated by the recovery cross-attention below
and terminates in the CLS classification head.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/model_architecture_reference.png}
\caption{The overall \mn{} architecture from the project model design (slide 3).
The local encoder produces the full-grid representation; dynamic merging and
the differentiable Top-$K$ path determine which information is routed into the
latent sequence through cross-attention. The local decoder shown in the design
is the recovery branch; for the ImageNet classifier, the latent path terminates
in the CLS head described below.}

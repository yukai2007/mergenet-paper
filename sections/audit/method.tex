...
We use $\tau_1=1$ and multiplicative soft-selection scale $\alpha=30$.
The released threshold routine uses
...
The distinction between method and checkpoint execution matters here. The
reported ImageNet checkpoints implement Equation~\eqref{eq:mass-attn} through
PyTorch fused scaled-dot-product attention with an additive mask. The
query/key augmentation is present in the released operator library and has
forward/backward parity tests, but it was not the latent-attention path used
to train those checkpoints. We consequently treat its latency as a separate
operator ablation rather than attributing it to the main ImageNet results.

...
exponential moving average (EMA) model. The September 23 packet contains 24
runs with recorded epochs: 23 complete schedules and a 140/150 weight-decay
probe that was deliberately abandoned. Endpoints in this manuscript are
recomputed from each run's \texttt{summary.csv} by header name. A refreshed
audit verifies 3,560 contiguous summary rows across all 24 runs and matches
3,354 rows' EMA loss, top-1 and top-5 to supplied logs. Logs are missing for
86 curriculum epochs (64--149), all 15 MergeNet 512-pixel epochs, and the first
105 epochs of the host-migrated DeiT-initialized run. These 206 rows remain

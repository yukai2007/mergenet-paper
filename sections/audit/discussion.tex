...
Interrupted initialization, progressive latent compression, and Base-scale training remain useful follow-up directions, but they are not required to support the present method claim. The submission depends only on two unavailable checkpoint-side outputs: a common-protocol numeric packet for the main table and component interventions, and passive routing traces for the final visualization. Keeping that boundary explicit prevents partial runs or unrelated future experiments from being presented as current evidence.

...
carriers survive the gather. This constraint preserves spatial proximity
without prescribing a fixed pooling assignment. We compare the resulting
routing variant with global, flattened-window, and degree-matched alternatives.
...
The experiments characterize the spatial-routing variant, with a clear limit
on the overall performance claim. In a single-seed, 150-epoch ImageNet-1K
study, radius-3 routing scores 1.670 percentage points above the global
variant and 0.598 points above the flattened-window variant. A three-seed
...
81.932\%; the uncompressed DeiT-S/8 reaches 82.246\%. The matched DTEM-p8
adaptation reaches \pendingvalue\% top-1. Together, the routing and
cross-method results establish the behavior of the physical bottleneck;
the common-protocol accuracy--latency comparison is
...
\begin{itemize}
  ...
  \item comparisons of original-grid routing with global, flattened-window,
  and degree-matched alternatives, including a three-seed CIFAR-100 study; and

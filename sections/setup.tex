...
\subsection{Protocol for reserved extensions}
\label{sec:pending-protocol}

The working-draft slots P01--P12 specify additional evidence without adding
observations to the completed-run tables. Initialization and progressive
latent experiments are defined in Sections~\ref{sec:pending-init}
and~\ref{sec:pending-progressive}; their resolved arguments and source-state
receipts must be fixed before interpretation. ImageNet support replication
uses paired training seeds (Section~\ref{sec:pending-geometry}). Joint
accuracy and latency require the configuration identity in
Section~\ref{sec:pending-frontier}. Additional metadata and currently missing
log segments are enumerated in Appendix~\ref{sec:pending-receipts}.

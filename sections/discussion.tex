...
predictions for paired significance tests. Appendix~\ref{app:followup}
provides predictions for two independently replayed local CIFAR checkpoints
from an earlier campaign; Section~\ref{sec:cifar-seeds} adds three-seed
training on resized CIFAR-100, which is not native-resolution ImageNet.
...
Two ImageNet runs in the September 19 packet are unfinished. The weight-decay
probe stopped at epoch 140. A DeiT-initialized \mn{} run reached epoch 104 of
150 and should be resumed from its last checkpoint; we do not treat its
partial curve as an endpoint, because earlier mid-training leads in this
campaign have decayed. Progressive latent merging was queued and not started.
A DeiT-B/16 300-epoch run overfits under drop-path 0.1 (Appendix~\ref{app:deitb})
and is not a usable base-scale anchor.

The most informative remaining experiments follow directly from these limits.
Resume the interrupted DeiT-initialized run to a 150-epoch endpoint, and run
the queued progressive-latent configuration as its own family. Any new local
ImageNet training must be reported as a separate experiment from the company
packet. Repeat the global, flattened, radius-3 and degree-matched controls at
additional ImageNet seeds only after the representation-level tests are
complete. The layout follow-up supplies bounded parity and complete-model
inference timing; complete training-step timing and long-run behavior remain
open. These experiments would test concrete explanations for the observed
gaps; merely extending a schedule based on a short terminal slope would not.
